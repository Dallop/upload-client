...
export const createNewArray = (size, value) => Array(size).fill(value)
...
const getAvailabilitiesFromModel = model => {
...
export const dayModelToString = model =>
  getAvailabilitiesFromModel(model).map(
...
export const dayModelToTimeObj = model => {
  const availabilities = getAvailabilitiesFromModel(model)
  return availabilities.map(tuple => ({
    startTime: indexToTimeObj(tuple[0]),
    endTime: indexToTimeObj(tuple[1])
  }))
}
export const weekModelToTimeObj = weekModel => weekModel.map(dayModelToTimeObj)
